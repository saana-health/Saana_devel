## 8/12/2019
## Script for processing and inputting Colruyt Demo Meals
...
PATH = os.path.join(os.getcwd(), 'csv/Colruyt/')
...
def processIngredients(filename):
    with open(filename) as csvfile:
        ...
        for row in reader_list:
            ...
            if name != '':
                if is_new_meal:
                    new_meal = Meal(name = name, supplierID = connectMongo.db.users.find_one({'first_name':'Colruyt'})['_id'])
                    is_new_meal = False
                else:
...
def processNutrition(filename):
    with open(filename) as csvfile:
        ...
        reader_list = list(csv.reader(csvfile))
        nutrition = {}
        columns = [x.lower() for x in reader_list[0][:9]]
