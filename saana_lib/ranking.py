...
class RankingOut(OutIn):
    ...
    def store(self, limit=constants.RECIPES_TO_RANK):
        ...
        patient_rec = {
            ...
            'recipe': recipes_all,
            'is_deleted': False,
            'created_at': datetime.utcnow().replace(tzinfo=datetime.timezone.utc).astimezone().replace(microsecond=0).isoformat(),
            'updated_at': datetime.utcnow().replace(tzinfo=datetime.timezone.utc).astimezone().replace(microsecond=0).isoformat()
            }
